...
class IncomingMail extends Controller
{
    ...
    {
    	 $getemailcredentials = DB::table('emailuser')
        ->select('emailuser_email','emailuser_emailpassword','emailuser_emailhost')
        ->where('emailuser_id','=',$request->user_id)
        ->first();
        if (empty($getemailcredentials)) {
        	return response()->json(['message' => 'Email Credentials Required'],400);
        }
        $_ENV['IMAP_HOST'] = $getemailcredentials->emailuser_emailhost;
        $_ENV['IMAP_USERNAME'] = $getemailcredentials->emailuser_email;
        $_ENV['IMAP_PASSWORD'] = $getemailcredentials->emailuser_emailpassword;
        ...
        $oClient = new \Client([
        'host'          => $getemailcredentials->emailuser_emailhost,
        'port'          => 993,
        'encryption'    => 'ssl',
        'validate_cert' => true,
        'username'      => $getemailcredentials->emailuser_email,
        'password'      => $getemailcredentials->emailuser_emailpassword,
        'protocol'      => 'imap'
        ]);
        $oclient->connect();
    ...
    public function receiveemaillist(Request $request)
    {
        $getemails = DB::table('receiveemail')
        ->select('*')
        ->where('emailmaster_token','=',$request->emailmaster_token)
        ->where('status_id','=',1)
        ->orderBy('receiveemail_id','DESC')
        ->get();
     	return response()->json(['data' => $getemails,'message' => 'Email Received'],200);
    }   
}
